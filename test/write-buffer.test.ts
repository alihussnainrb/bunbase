...
describe('WriteBuffer', () => {
	describe('pushLog()', () => {
		it('should add log to buffer', () => {
			const buffer = new WriteBuffer({ flushIntervalMs: 10000, maxBufferSize: 100 })
		...
		it('should trigger early flush when buffer reaches max size', async () => {
			...
			const buffer = new WriteBuffer({ flushIntervalMs: 10000, maxBufferSize: 2 })
			...
			buffer.pushLog({ run_id: 'r1', level: 'info', message: 'm1', meta: null, created_at: 1 })
			buffer.pushLog({ run_id: 'r2', level: 'info', message: 'm2', meta: null, created_at: 2 })
	...
	describe('flush()', () => {
		it('should clear buffers after flush', async () => {
			...
			buffer.pushLog({ run_id: 'r1', level: 'info', message: 'm', meta: null, created_at: 1 })
			buffer.pushRun({ id: '1', action_name: 'test', module_name: null, trace_id: 't1', trigger_type: 'api', status: 'success', input: '{}', output: '{}', error: null, duration_ms: 1, started_at: 1 })
	...
	describe('shutdown()', () => {
		it('should flush remaining entries', async () => {
			const buffer = new WriteBuffer({ flushIntervalMs: 60000, maxBufferSize: 1000 })
		...
		it('should clear flush interval', async () => {
			const buffer = new WriteBuffer({ flushIntervalMs: 100, maxBufferSize: 1000 })
	...
	describe('options', () => {
		...
		it('should accept custom flush interval', async () => {
			const buffer = new WriteBuffer({ flushIntervalMs: 50, maxBufferSize: 1000 })
