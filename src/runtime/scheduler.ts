...
export class Scheduler {
    ...
    private delayedTasks = new Map<string, { timeout: ReturnType<typeof setTimeout>; handler: () => void }>()
    ...
    async schedule(
        ...
        if (typeof time === 'number') {
            ...
            this.logger.info(`[Scheduler] Scheduled ${name} in ${time}s`, { taskId: id })
            ...
            this.delayedTasks.set(id, { timeout, handler })
            this.logger.info(`[Scheduler] Scheduled ${name} at ${time.toISOString()}`, { taskId: id })
            ...
            this.logger.info(`[Scheduler] Scheduled cron ${name}: ${time}`, { taskId: id })
        }
