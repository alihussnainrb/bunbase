...
type DatabaseClient<DB extends Database = Database> = {
  from: <T extends keyof DB['public']['Tables']>(table: T) => new TypedQueryBuilder<DB['public']['Tables'][T]>(table as string),

  raw: (strings: TemplateStringsArray, ...values: any[]) =>
    sqlPool(strings, ...values),

  transaction: sqlPool.begin.bind(sqlPool),
}
...
export function createDB<DB extends Database = Database>() {
  ...
  return {
    from: <T extends keyof Tables>(table: T) => new TypedQueryBuilder<Tables[T]>(table as string),
...
class TypedQueryBuilder<Table extends Database['public']['Tables'][string]> {
  ...
  select<
    ...
    Result = Fields extends '*' ? Table['Row'] : Pick<Table['Row'], Fields extends keyof Table['Row'] ? Fields : never>
  >(...fields: Fields[]): TypedQueryBuilder<Table> & { exec: () => Promise<Result[]> } {
    this.selects = fields.length ? (fields as string[]) : ['*'];
    return this as any;
  }

  eq<K extends keyof Table['Row']>(column: K, value: Table['Row'][K]) {
  ...
  // Example:
  in<K extends keyof Table['Row']>(column: K, values: Table['Row'][K][]) {
  ...
  returning<Fields extends keyof Table['Row']>(fields: Fields[]) {
