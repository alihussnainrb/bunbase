...
`
		...
		// Write output
		const outDir = join(process.cwd(), outputPath.split('/').slice(0, -1).join('/'))
...
async function typegenReact(opts: TypegenReactOptions): Promise<void> {
	...
	try {
		...
		for (const [path, methods] of Object.entries(openapi.paths)) {
			for (const [method, operationData] of Object.entries(methods as any)) {
				...
				if (operation.requestBody?.content?.['application/json']?.schema) {
					...
					inputType = generateTypeFromSchema(schema, openapi.components?.schemas)
				}
				...
				if (operation.responses?.['200']?.content?.['application/json']?.schema) {
					...
					outputType = generateTypeFromSchema(schema, openapi.components?.schemas)
				}
...
`
		...
		console.log(`✓ Generated ${actions.length} action type(s) -> ${output}`)
		console.log('')
		console.log('Import in your React app:')
		console.log(`  import type { BunbaseAPI } from '${output.replace('.d.ts', '')}'`)
	} catch (error) {
