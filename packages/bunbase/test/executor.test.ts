...
describe('executeAction()', () => {
	...
	it('should execute action successfully and return data', async () => {
		...
		const result = await executeAction(registered, { name: 'World' }, {
			triggerType: 'api',
	...
	it('should run guards in sequence before handler', async () => {
		...
		const guard1 = async () => { guardOrder.push('guard1') }
		const guard2 = async () => { guardOrder.push('guard2') }
		...
		await executeAction(registered, {}, {
			triggerType: 'api',
			logger: mockLogger as any,
			writeBuffer: mockWriteBuffer,
		})
	...
	it('should stop execution when guard throws', async () => {
		...
		const failingGuard = async () => { throw guardError }
		...
		const result = await executeAction(registered, {}, {
			triggerType: 'api',
			logger: mockLogger as any,
			writeBuffer: mockWriteBuffer,
		})
	...
	it('should write run entry on success', async () => {
		...
		const mockBuffer = {
			...mockWriteBuffer,
			pushRun: (entry: any) => { pushRunCalls.push(entry) },
		}
		...
		await executeAction(registered, { value: 5 }, {
			triggerType: 'api',
			logger: mockLogger as any,
