...
import { parsePathParams, parseQueryParams, matchViewPath } from '../views/url-parser.ts'
import { renderJSX, html } from '../views/view.ts'
...
export class BunbaseServer {
    ...
    private async handleView(view: any, req: Request): Promise<Response> {
        ...
        try {
            ...
            return new Response(html('Error', `<div class="p-6 text-red-600">Error: ${message}</div>`, this.viewsConfig), {
                status: 500,
                headers: { 'Content-Type': 'text/html' },
            })
        }
