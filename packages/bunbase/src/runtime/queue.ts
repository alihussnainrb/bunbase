...
export class Queue {
    ...
    async push(name: string, data: unknown, opts?: QueueOptions): Promise<string> {
     ...
     */
    async update(jobId: string, updates: { data?: unknown; priority?: number }): Promise<boolean> {
        if (updates.data !== undefined) {
            await this.sql`UPDATE job_queue SET data = ${JSON.stringify(updates.data)} WHERE id = ${jobId}`
        }
        if (updates.priority !== undefined) {
            await this.sql`UPDATE job_queue SET priority = ${updates.priority} WHERE id = ${jobId}`
        }
    ...
    async start(): Promise<void> {
        ...
        this.running = true
        this.logger.info('[Queue] Started worker', { pollIntervalMs: this.pollIntervalMs })
    ...
    private async poll(): Promise<void> {
        ...
        try {
            ...
            if (!handler) {
                await this.markFailed(job.id, `No handler registered for job: ${job.name}`)
                return
            }
    ...
    private async executeJob(job: Job, handler: JobHandler): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`[Queue] Job ${job.name} failed:`, { jobId: job.id, error: errorMessage })
    ...
    async retryFailedJob(jobId: string): Promise<string | null> {
        ...
        this.logger.info(`[Queue] Retried job ${failed.name} from dead letter queue`, { jobId: result.id })
        return result.id
    }
    ...
    async getStats(): Promise<{
        ...
        const [deadStats] = await this.sql`SELECT COUNT(*) as count FROM job_failures`
    ...
    async listJobs(opts: {
        ...
        query = this.sql`${query} ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`
