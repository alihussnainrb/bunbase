...
import type { OpenApiBuilder, OperationObject, ResponseObject, PathItemObject } from 'openapi3-ts/oas31'
    ...
    for (const action of registry.getAll()) {
        for (const trigger of action.triggers) {
            if (trigger.type === 'api') {
                ...
                const method = trigger.method.toLowerCase() as 'get' | 'post' | 'put' | 'patch' | 'delete'
 ...
 */
export function generateScalarDocs(spec: ReturnType<OpenApiBuilder['getSpec']>): string {
